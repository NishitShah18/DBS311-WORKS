...
SET SERVEROUTPUT ON;
-- for outputting errors for testing purposes

-- find_customer
CREATE 
OR REPLACE PROCEDURE find_customer(
  customerId IN NUMBER, found OUT NUMBER
) AS BEGIN 
SELECT 
  COUNT(*) INTO found 
FROM 
...
EXCEPTION WHEN NO_DATA_FOUND THEN found := 0;
END;
/ 
...
EXCEPTION WHEN NO_DATA_FOUND THEN price := 0;
END;
/ 
...
VALUES 
  (
    orderId, itemId, productId, inQuantity, 
    price
  );
END;
/ 
...
OR REPLACE PROCEDURE add_order(
  customerId IN NUMBER, new_order_id OUT NUMBER
) AS BEGIN 
SELECT 
  MAX(order_id) INTO new_order_id 
FROM 
  orders;
...
VALUES 
  (
    new_order_id, customerId, 'Shipped', 
    56, SYSDATE
  );
END;
/